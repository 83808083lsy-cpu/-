...
local function DoRagebot()
    ...
    if SCAN.Mode == "Original" then
        if WB.Round % 2 == 0 then
            -- 【雙數輪】：使用 解析1 (25點固定向量 + 隨機旋轉)
            ...
            local oMag = oPole.Magnitude
            if oMag < 0.001 then
                oPole = Vector3.new(0,0,1)
            else
                oPole = oPole.Unit
            end
            ...
            if WB.Round % 4 == 0 then
                ...
                for i,v in ipairs(newOrigin) do
                    if (v - forward_offset).Magnitude < 0.001 then inserted = true; break end
                end
                if not inserted then table.insert(newOrigin, forward_offset) end
            end

            newTarget = { tPos }
            local hPole = -oPole
            local arb = math.abs(hPole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
            local t1 = hPole:Cross(arb)
            if t1.Magnitude == 0 then
                arb = Vector3.new(0,0,1)
                t1 = hPole:Cross(arb)
            end
            t1 = t1.Unit
            local t2 = hPole:Cross(t1).Unit
            for i = 1, Hit_Scans do
                local theta = math.random() * 2 * math.pi
                local r = Hit_Radius * math.sqrt(math.random())
                local offsetVec = t1 * (math.cos(theta) * r) + t2 * (math.sin(theta) * r)
                table.insert(newTarget, tPos + offsetVec)
            end
        end

    elseif SCAN.Mode == "RandomSphere" then
        local rradius = SCAN.RandomRadius or 25
        if rradius > 25 then rradius = 25 end -- 强制上限25
        if REDUCE_RANDOM then rradius = math.max(1, math.floor(rradius/2)) end
        newOrigin = GetOffsets_Sphere(myPos, rradius, Origin_Scans)
        ...
        local arb = math.abs(hPole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
        local t1 = hPole:Cross(arb)
        if t1.Magnitude == 0 then arb = Vector3.new(0,0,1); t1 = hPole:Cross(arb) end
        t1 = t1.Unit
        local t2 = hPole:Cross(t1).Unit
        newTarget = { tPos }
        for i = 1, Hit_Scans do
            local theta = math.random() * 2 * math.pi
            local r = Hit_Radius * math.sqrt(math.random())
            table.insert(newTarget, tPos + (t1 * (math.cos(theta)*r) + t2 * (math.sin(theta)*r)))
        end

    elseif SCAN.Mode == "Cardinals" then
        newOrigin = GetOffsets_Cardinals(myPos, Origin_Radius)
        newTarget = GetOffsets_Cardinals(tPos, Hit_Radius)

    elseif SCAN.Mode == "Aggressive" then
        -- Aggressive: 更高密度 + 混合策略
        local extraO = math.min(Origin_Scans * 2, 120)
        ...
        for i = 1, extraH do
            local theta = math.random() * 2 * math.pi
            local r = Hit_Radius * math.sqrt(math.random()) * (1 + math.random()*0.3)
            table.insert(newTarget, tPos + (t1 * (math.cos(theta)*r) + t2 * (math.sin(theta)*r)))
        end
    ...
    elseif SCAN.Mode == "Hybrid" then
        ...
        for i = 1, math.max(4, Origin_Scans//2) do
            local off = GetOffsets_Algo2(myPos, oPole, Origin_Radius*0.6, 1)
            for _,v in ipairs(off) do table.insert(newOrigin, v) end
        end
        newTarget = GetOffsets_Algo2(tPos, -oPole, Hit_Radius, Hit_Scans)
        for _,v in ipairs(GetOffsets_Cardinals(tPos, Hit_Radius*0.5)) do table.insert(newTarget, v) end

    elseif SCAN.Mode == "Focused" then
        -- Focused: 小半径高密度（优先中心与前置方向），适合精确单点打击
        local pole = (tPos - myPos)
        if pole.Magnitude < 0.001 then pole = Vector3.new(0,0,1) else pole = pole.Unit end
        newOrigin = { myPos, myPos + pole * math.min(Origin_Radius*0.6, 10) }
        for i=1, Origin_Scans do
            local theta = math.random()*2*math.pi
            local r = (Origin_Radius*0.35) * math.sqrt(math.random())
            local arb = math.abs(pole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
            local t1 = pole:Cross(arb); if t1.Magnitude==0 then t1 = pole:Cross(Vector3.new(0,0,1)) end; t1=t1.Unit
            local t2 = pole:Cross(t1).Unit
            table.insert(newOrigin, myPos + t1*(math.cos(theta)*r) + t2*(math.sin(theta)*r))
        end
        newTarget = { tPos }
        for i=1, Hit_Scans*2 do
            local theta = math.random()*2*math.pi
            local r = (Hit_Radius*0.5) * math.sqrt(math.random())
            local arb = math.abs(pole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
            local t1 = (-pole):Cross(arb); if t1.Magnitude==0 then t1 = (-pole):Cross(Vector3.new(0,0,1)) end; t1=t1.Unit
            local t2 = (-pole):Cross(t1).Unit
            table.insert(newTarget, tPos + t1*(math.cos(theta)*r) + t2*(math.sin(theta)*r))
        ...
        -- fallback to original alternating
        if WB.Round % 2 == 0 then
            newOrigin = GetOffsets_Algo1(myPos, tPos, Origin_Radius)
            newTarget = GetOffsets_Algo1(tPos, myPos, Hit_Radius)
        else
            local oPole = (tPos - myPos)
            local oMag = oPole.Magnitude
            if oMag < 0.001 then oPole = Vector3.new(0,0,1) else oPole = oPole.Unit end
            newOrigin = GetOffsets_Algo2(myPos, oPole, Origin_Radius, Origin_Scans)
            newTarget = { tPos }
            local hPole = -oPole
            local arb = math.abs(hPole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
    ...
    if bestPO and bestPH then
        ...
        if not refined then
            for i = 1, 12 do
                local dirToTarget = (tPos - bestPO)
                if dirToTarget.Magnitude <= 0 then break end
                local tryPO = bestPO + dirToTarget.Unit * (i * 0.2)
                if CheckWallbang(tryPO, bestPH) then bestPO = tryPO; refined = true; break end
            end
        end
