...
-- 新增扫描模式设置（Cardinals / Random / Dual）
local SCAN_SETTINGS = {
    Cardinals_Enabled = true,  -- Cardinals 扫描默认启用
    Cardinals_Radius  = 20,    -- Cardinals 半径默认 20
    Random_Enabled    = true,  -- Random 扫描默认启用
    Random_Radius     = 30,    -- Random 半径默认 30
    DualScan          = true,  -- 双扫描（同时启用两种扫描）
}
...
do -- Combat page
    ...
    do -- Ragebot
      ...
      s1:Slider({Name="Max Cache", Flag="CAT_Max_Cache_30",    Min=0.1, Max=25,  Default=0.5,   Decimals=0.01, Callback=function(v) WB.Threshold=v end})
        ...
        -- 新增：Cardinals / Random / Dual Scan 设置
        s1:Toggle({Name="Cardinals Scan", Flag="CAT_Cardinals_74", Default=true, Callback=function(v) SCAN_SETTINGS.Cardinals_Enabled=v end})
        s1:Slider({Name="Cardinals Radius", Flag="CAT_Cardinals_Rad_75", Min=1, Max=200, Default=20, Callback=function(v) SCAN_SETTINGS.Cardinals_Radius=v end})
        s1:Toggle({Name="Random Scan", Flag="CAT_Random_76", Default=true, Callback=function(v) SCAN_SETTINGS.Random_Enabled=v end})
        s1:Slider({Name="Random Radius", Flag="CAT_Random_Rad_77", Min=1, Max=400, Default=30, Callback=function(v) SCAN_SETTINGS.Random_Radius=v end})
        s1:Toggle({Name="Dual Scan", Flag="CAT_DualScan_78", Default=true, Callback=function(v) SCAN_SETTINGS.DualScan=v end})
...
-- == 改动说明：
-- ==  - 采样在与 poleDir 垂直的平面上进行（即在目标的“屏幕平面”上），
-- ==  - 使用 concentric disk (r = radius * sqrt(rand)) 来保证圆盘内的均匀分布，
-- ==  - 这样 radius 始终是世界单位，不会随距离缩减，命中更精确稳定。
-- =====================================================================
local function GetOffsets_Algo2(center, poleDir, radius, count)
    ...
    -- 为了提高命中精度：中心附近点权重更高（可选）
    -- 使用 concentric disk sampling 保证圆盘上的均匀采样
    for i = 1, count do
        -- 极坐标：角度与半径（半径使用 sqrt(rand) 保证面积均匀）
        local theta = (2 * math.pi) * (i / count) + (math.random() - 0.5) * (2*math.pi / count) -- 带少量角度抖动
...
-- == 说明：
-- ==  - 删除会在平地/无掩体时把偏移设置为零的逻辑，保持原有响应速度
-- ==  - 减少 19 单位前向偏移被优先使用的频率（仅每 4 轮尝试一次）
-- ==  - 如果找到可用偏移点，会在该点附近做持续/局部精化扫描并在后续轮次优先验证与精化
-- ==  - 命中点如果是采样点，则在最终射击时在该采样点附近做小幅随机（在需要的范围内随机化）
-- ==  - 新增：保存并优先使用上一次有效轨迹的命中半径；若无法采样半径则回退到最外环（Hit_Radius）
-- =====================================================================
local function DoRagebot()
    ...
    -- 预先计算方向基（用于 Card/Random 注入）
    local oPole = (tPos - myPos)
    if oPole.Magnitude < 0.001 then oPole = Vector3.new(0,0,1) else oPole = oPole.Unit end
    local hPole = -oPole
    local o_arb = math.abs(oPole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
    local o_t1 = oPole:Cross(o_arb)
    if o_t1.Magnitude == 0 then o_arb = Vector3.new(0,0,1); o_t1 = oPole:Cross(o_arb) end
    o_t1 = o_t1.Unit
    local o_t2 = oPole:Cross(o_t1).Unit
    local h_arb = math.abs(hPole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
    local h_t1 = hPole:Cross(h_arb)
    if h_t1.Magnitude == 0 then h_arb = Vector3.new(0,0,1); h_t1 = hPole:Cross(h_arb) end
    h_t1 = h_t1.Unit
    local h_t2 = hPole:Cross(h_t1).Unit
    ...
    if Locked_Path then
        ...
        if dO<=WB.Threshold and dH<=WB.Threshold and inRange then
            -- 如果原先路径仍然可用，优先复用，但同时做局部精化（持续扫描附近）
            if CheckWallbang(Locked_Path.AbsO,Locked_Path.AbsH) then
                -- 延长快速扫描窗口，使后续若干帧内优先验证/精化该点（提高反应速度）
                Locked_Path.FastScanUntil = tick() + 0.6

                -- 局部精化：在命中点周围做少量随机采样以寻找更稳定或更优的点
                local refined = false
    ...
    if WB.Round % 2 == 0 then
        ...
        -- 【單數輪】：使用 解析2 (圓盤均勻采樣，確保 radius 為固定世界單位)
        local oPole = (tPos - myPos)
        local oMag = oPole.Magnitude
        if oMag < 0.001 then
            -- 使用一個默認前向向量繼續采樣（避免直接返回/置零）
            oPole = Vector3.new(0,0,1)
        else
            oPole = oPole.Unit
        end
        local hPole = -oPole -- 敵人的半球朝向玩家
        
        -- 優化：減少 19 單位前向偏移被優先使用的頻率（僅每 4 輪嘗試一次）
        ...
        -- 將 forward_offset 放到備選列表末尾（低優先級），並僅在每 4 輪嘗試一次加入
        ...
        -- 對命中點採用完全隨機采樣（在目標的屏幕平面圓盤內隨機點）
        newTarget = { tPos } -- 保證中心點首先被檢測
        -- 構建與 poleDir 垂直的平面基向量
    ...
    -- ========== 注入 Cardinals / Random 扫描候选点（基于 SCAN_SETTINGS） ==========
    do
        -- 确保 newOrigin/newTarget 存在
        newOrigin = newOrigin or { myPos }
        newTarget = newTarget or { tPos }

        -- 如果启用了 Cardinals 或 DualScan，则在目标/发射点附近加入四个正交点
        if SCAN_SETTINGS.Cardinals_Enabled or SCAN_SETTINGS.DualScan then
            -- 对发射点使用 o_t1/o_t2, 对命中点使用 h_t1/h_t2
            local r = SCAN_SETTINGS.Cardinals_Radius or 20
            -- origin cardinals
            table.insert(newOrigin, myPos + o_t1 * r)
            table.insert(newOrigin, myPos - o_t1 * r)
            table.insert(newOrigin, myPos + o_t2 * r)
            table.insert(newOrigin, myPos - o_t2 * r)
            -- target cardinals
            table.insert(newTarget, tPos + h_t1 * r)
            table.insert(newTarget, tPos - h_t1 * r)
            table.insert(newTarget, tPos + h_t2 * r)
            table.insert(newTarget, tPos - h_t2 * r)
        end

        -- 如果启用了 Random 或 DualScan，则在目标/发射点周围加入若干随机样本
        if SCAN_SETTINGS.Random_Enabled or SCAN_SETTINGS.DualScan then
            local r = SCAN_SETTINGS.Random_Radius or 30
            local count = 6
            for i = 1, count do
                local theta = math.random() * 2 * math.pi
                local rr = r * math.sqrt(math.random())
                local offsetVecT = h_t1 * (math.cos(theta) * rr) + h_t2 * (math.sin(theta) * rr)
                table.insert(newTarget, tPos + offsetVecT)
                -- 对发射点也做少量随机注入（更丰富的轨迹尝试）
                local theta2 = math.random() * 2 * math.pi
                local rr2 = (r * 0.5) * math.sqrt(math.random())
                local offsetVecO = o_t1 * (math.cos(theta2) * rr2) + o_t2 * (math.sin(theta2) * rr2)
                table.insert(newOrigin, myPos + offsetVecO)
            end
        end
    end
    -- ========================================================================

    -- 4. 雙重循環窮舉尋找合法彈道 (找到即返回，不再比較距離)
    ...
    for _, pO in ipairs(newOrigin) do
        ...
        if bestPO then break end -- 找到就跳出外層循環
    end

    -- 5. 局部精化：如果找到初步路徑，嘗試在該路徑附近做更密集的隨機掃描以提高穩定性
    if bestPO and bestPH then
        ...
        -- 最終處理：若以上都沒找到更穩定點，但原始 bestPO/bestPH 仍有效，則保留；否則放棄
        if refined then
...
RunService.Heartbeat:Connect(function()
    ...
    -- 选择最终命中点：
    -- 优先：如果上一次锁定的轨迹现在仍然有效（CheckWallbang），则直接使用该轨迹，不做随机化（保证连贯伤害轨迹）
    ...
    if Valid_Pair and Valid_Pair.Origin and Valid_Pair.Hit and CheckWallbang(Valid_Pair.Origin, Valid_Pair.Hit) then
        ...
        -- 否则尝试自适应随机化：优先用 Valid_Pair.HitRadiusUsed（若存在），否则回退到 Hit_Radius（最外环）
        if Valid_Pair and Valid_Pair.Target and Valid_Pair.Target.Character then
            ...
            if pole.Magnitude > 0.001 then
                ...
                if not found then
                    -- fallback attempts with decreasing r
                    for j=1,3 do
