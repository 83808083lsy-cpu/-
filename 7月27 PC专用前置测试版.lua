    ...
    SC.AUS_Loop = task.spawn(function()
        while SC.AUS_Enabled do
            ...
            if hrp and hum then
                ...
                if bredMakurz then
                    local closestSafe, minDist = nil, 3  -- 加大感应距离到 12
...
local Watermark = Library:Watermark("Link.cc")
local KeybindList = Library:KeybindList()

-- 新增扫描模式配置表（默认 Original，RandomSphere 半径默认 25，上限 25）
SCAN = {
    Mode = "Original",       -- "Original" | "RandomSphere" | "Cardinals" | "Aggressive" | "Hybrid" | "Focused"
    RandomRadius = 25,       -- 1..25 可由 UI 控制，仅在 RandomSphere 下使用（上限已改为25）
}

-- ========== 新增功能配置 ==========
HIT_RECORD = {
    Enabled = false,        -- 是否保存命中记录到本地文件
    Folder  = "sa_hit_records",
}
WB.CacheHitTolerance = 0.3 -- 缓存命中容差（世界单位），固定为0.3（不再由滑块控制）
WB.CacheHitToleranceEnabled = true -- 默认开启（改为开关，无滑块）

EXTRAPOLATE = {
    Enabled = false,
    MS = 30, -- 默认 30, 最大 60 (表示外推毫秒数，外推位移 = vel * (MS/1000))
}

REDUCE_RANDOM = false -- 尽量不随机偏移（优先确定性采样）
EXPAND_HIT_RADIUS = false -- 启用采样扩张（采样上限使用30，但最终发射半径 clamp 到 25）

-- 新增开关：预加载/模拟偏移点（基于目标速度/自身坐标提前推理并预计算）
PRELOAD_SIM_ENABLED = false
-- 新增开关：快速扫描轨迹有效性（短时高频验证锁定轨迹）
FAST_VALIDATE_ENABLED = false
-- 新增开关：在射击时也快速扫描以验证命中点
SCAN_ON_FIRE_ENABLED = false
...
do -- Combat page
    ...
    do -- Ragebot
      ...
      s1:Slider({Name="Max Cache", Flag="CAT_Max_Cache_30",    Min=0.1, Max=25,  Default=0.5,   Decimals=0.01, Callback=function(v) WB.Threshold=v end})
        ...
        -- 新增：扫描模式控制（Original / Random Sphere / Cardinals / Aggressive / Hybrid / Focused）
        s1:Dropdown({Name="Scan Mode", Flag="CAT_Scan_Mode", Items={"Original","Random Sphere","Cardinals","Aggressive","Hybrid","Focused"}, Default="Original", Callback=function(v)
            if v == "Original" then
                SCAN.Mode = "Original"
            elseif v == "Random Sphere" then
                SCAN.Mode = "RandomSphere"
            elseif v == "Cardinals" then
                SCAN.Mode = "Cardinals"
        ...
        -- 新增：随机球体半径滑块（仅在 Random Sphere 下有效，最大改为25）
        s1:Slider({Name="Random Sphere Radius", Flag="CAT_Random_Radius", Min=1, Max=25, Default=25, Callback=function(v) SCAN.RandomRadius = v end})

        -- 新功能 UI 控件
        s1:Toggle({Name="Save hit records", Flag="CAT_Save_Hits", Default=false, Callback=function(v) HIT_RECORD.Enabled = v end})
        -- Cache Hit Tolerance 改为开关（默认开启），移除滑块
        s1:Toggle({Name="Cache Hit Tolerance (0.3)", Flag="CAT_Cache_Tol", Default=true, Callback=function(v) WB.CacheHitToleranceEnabled = v end})

        s1:Toggle({Name="Extrapolate Position", Flag="CAT_Extrapolate", Default=false, Callback=function(v) EXTRAPOLATE.Enabled = v end})
        s1:Slider({Name="Extrapolate (ms)", Flag="CAT_Extrapolate_MS", Min=10, Max=60, Default=30, Callback=function(v) EXTRAPOLATE.MS = v end})

        s1:Toggle({Name="Reduce randomization", Flag="CAT_Reduce_Random", Default=false, Callback=function(v) REDUCE_RANDOM = v end})
        s1:Toggle({Name="Expand Hit Radius (sampling)", Flag="CAT_Expand_Hit", Default=false, Callback=function(v) EXPAND_HIT_RADIUS = v end})

        -- 新增功能开关（UI 控件）
        s1:Toggle({Name="Preload Simulate Offsets", Flag="CAT_Preload_Sim", Default=false, Callback=function(v) PRELOAD_SIM_ENABLED = v end})
...
local function safe_makefolder(path)
    pcall(function() if makefolder then makefolder(path) end end)
end
local function safe_writefile(path, content)
    return pcall(function() if writefile then writefile(path, content) end end)
end

local function SaveHitRecord(targetName, recordTable)
    if not HIT_RECORD.Enabled then return end
    local ok, ok2 = pcall(function() safe_makefolder(HIT_RECORD.Folder) end)
    local fname = HIT_RECORD.Folder.."/"..(targetName or "unknown").."_"..tostring(os.time())..".txt"
    local lines = {}
    ...
    pcall(function() safe_writefile(fname, content) end)
end
...
local ALGO1_MAX_ROLL_DEG = 60 -- 提升到 60 度以增加覆盖
...
local function GetOffsets_Algo1(firePos, targetPos, offset)
    ...
    if not rotatedUnits then
        ...
        for i=1,#ScanVectorsUnit do
            ...
            local lateralScale = useLateral and 1.7 or 1.0
            local forwardScale = useLateral and 0.25 or 1.0
...
local function GetOffsets_Algo2(center, poleDir, radius, count)
    ...
    -- 主轮：Vogel 分布（确定性、高效），并增加稳定性：对小 count 增加次级环绕采样
    ...
    -- 次级环：在 radius 的 0.5, 0.75 处各加少量样本以增强命中稳定性
    local extraRings = math.min(3, math.max(1, math.floor(baseCount / 8)))
    for ring = 1, extraRings do
        local rr = radius * (0.5 + (ring / (extraRings*2)))
        for k = 1, math.max(3, math.floor(baseCount/6)) do
            local th = (k / (math.max(3, math.floor(baseCount/6))+1)) * 2*math.pi + (ring * 0.123)
            local x = math.cos(th) * rr
            local y = math.sin(th) * rr
            table.insert(offsets, center + t1 * x + t2 * y)
        end
    end
...
-- =====================================================================
-- == 新增：球体均匀体积采样（Random Sphere 模式使用）
-- =====================================================================
local function GetOffsets_Sphere(center, radius, count)
    if not radius or radius <= 0 or count <= 0 then return {center} end
    ...
    for i = 1, count do
        ...
        local xy = math.sqrt(math.max(0, 1 - z*z))
        local x = xy * math.cos(theta)
        local y = xy * math.sin(theta)
...
-- =====================================================================
-- == 新增：Cardinals 模式：在固定向量组上以固定半径持续采样（无随机旋转）
-- =====================================================================
...
-- =====================================================================
-- == Helper: GetRandomOffset (用于局部扰动)
-- =====================================================================
local function GetRandomOffset(maxRadius)
    if not maxRadius or maxRadius <= 0 then return Vector3.zero end
...
-- =====================================================================
-- == 新增：更强的合并式采样函数（增强扫描与命中扫描）
-- == 目的：保留原有不同算法的行为但合并多策略、提升样本量与稳定性
-- =====================================================================
local function append_unique(dest, src)
    for _, v in ipairs(src) do
        local dup = false
        for _, w in ipairs(dest) do
            if (v - w).Magnitude < 1e-6 then dup = true; break end
        end
        if not dup then table.insert(dest, v) end
    end
end

local function StrongerScanOrigin(myPos, tPos, radius, scans, variant)
    -- variant: "Algo1Even"/"Algo2Odd"/nil
    local out = {myPos}
    scans = math.max(1, scans or 8)
    -- combine: Algo1 (fast directional), Algo2 (disk), Cardinals (fixed), Sphere (volumetric small)
    if not variant or variant == "Algo1Even" then
        append_unique(out, GetOffsets_Algo1(myPos, tPos, radius))
    end
    append_unique(out, GetOffsets_Algo2(myPos, (tPos - myPos).Unit, radius, math.max(1, math.floor(scans/1.5))))
    append_unique(out, GetOffsets_Cardinals(myPos, math.max(0.1, radius * 0.6)))
    append_unique(out, GetOffsets_Sphere(myPos, math.min(radius * 0.6, 12), math.min(6, math.floor(scans/3))))
    -- if variant requests more forward bias, insert forward offset
    if variant == "Algo2Odd" then
        local pole = (tPos - myPos)
        local poleUnit = (pole.Magnitude > 0.001) and pole.Unit or Vector3.new(0,0,1)
        table.insert(out, myPos + poleUnit * math.min(radius, 19.5))
    end
    return out
end

local function StrongerScanTarget(myPos, tPos, radius, scans, variant)
    local out = {tPos}
    scans = math.max(1, scans or 8)
    -- mix radial vogel, cardinals, sphere around target, and a few forward-biased samples
    append_unique(out, GetOffsets_Algo2(tPos, (tPos - myPos).Unit, radius, math.max(1, math.floor(scans/1.5))))
    append_unique(out, GetOffsets_Sphere(tPos, math.min(radius * 0.9, 16), math.min(16, math.floor(scans))))
    append_unique(out, GetOffsets_Cardinals(tPos, math.max(0.1, radius * 0.5)))
    -- add front small line of points towards shooter to cover body/neck offsets
    local pole = (tPos - myPos)
    local poleUnit = (pole.Magnitude > 0.001) and pole.Unit or Vector3.new(0,0,1)
    for i=1,4 do
        local dist = (i/4) * math.min(4, radius)
        table.insert(out, tPos + poleUnit * dist)
    end
    return out
end
...
local function DoRagebot()
    ...
    -- Extrapolate position if enabled
    if EXTRAPOLATE.Enabled and tRoot and tRoot:FindFirstChildOfClass("BasePart") then
        local vel = tRoot.Velocity or Vector3.zero
        local dt = (EXTRAPOLATE.MS or 0) / 1000
        tPos = tPos + vel * dt
    end

    -- PRELOAD_SIM: 预先基于速度推理偏移并提前验证（提高命中稳定性）
    if PRELOAD_SIM_ENABLED then
        local ok, vel = pcall(function() return tRoot and tRoot.Velocity or Vector3.zero end)
        vel = (ok and vel) and vel or Vector3.zero
        local pred_dt = 0.08 -- 80ms 预判
        local predPos = tPos + vel * pred_dt
        -- 在预测点周围做少量采样验证
        local pole = (predPos - myPos)
        if pole.Magnitude < 0.001 then pole = Vector3.new(0,0,1) else pole = pole.Unit end
        ...
        for i=1,try_count do
            local theta = math.random() * 2 * math.pi
            local r = math.min( (Hit_Radius or 4), 5 ) * math.sqrt(math.random())
            local offsetVec = t1 * (math.cos(theta) * r) + t2 * (math.sin(theta) * r)
            local cand = predPos + offsetVec
            if CheckWallbang(myPos, cand) then
                -- 记录预加载候选并立即使用（优先）
                PreloadedCandidate = {Origin = myPos, Hit = cand, Target = target, Sampled=true, HitRadiusUsed = r}
                Valid_Pair = PreloadedCandidate
                Locked_Path = {AbsO = myPos, AbsH = cand, Target = target, MyPos = myPos, TPos = tPos, RefineRadius = r, LastFound = tick(), FastScanUntil = tick() + 0.6, Sampled=true, HitRadiusUsed=r}
                WB.Cached = false
                return
            end
        end
    ...
    local FAST_SCAN_INTERVAL = 0.02 -- 快速扫描间隔（秒），用于锁定点的高频验证
    local baseInterval = ScanRate>0 and (1/ScanRate) or 0
    local minInterval = baseInterval
    if Locked_Path and Locked_Path.FastScanUntil and tick() < Locked_Path.FastScanUntil then
        minInterval = math.min(minInterval, FAST_SCAN_INTERVAL)
    end
    -- 如果启用了 FAST_VALIDATE，则允许更小的间隔（提高验证频率）
    if FAST_VALIDATE_ENABLED then
        minInterval = math.min(minInterval, FAST_SCAN_INTERVAL)
    end
    if tick()-WB.LastScan < minInterval then return end
    WB.LastScan=tick()
    WB.Round=WB.Round+1 -- 轮数+1，用于交替算法

    -- 3. 根据 SCAN.Mode 选择采样策略
    local newOrigin, newTarget
    
    local function clamp_final_fire_offset(origin, cand)
        -- 确保最终发射向量长度不超过25（当 EXPAND_HIT_RADIUS 时仍需限制实际发射）
        local maxLen = 25
        local vec = cand - origin
        local mag = vec.Magnitude
        if mag <= maxLen then return cand end
        if mag == 0 then return origin end
        return origin + vec.Unit * maxLen
    end

    -- === 替换/增强扫描逻辑：使用 StrongerScanOrigin/StrongerScanTarget 合并多策略采样（保留 Original 的单双轮交替语义） ===
    local mode = SCAN and SCAN.Mode or "Original"
    if mode == "Original" then
        if WB.Round % 2 == 0 then
            -- 双数轮 -> 偏向 Algo1 的强力采样（保留原有快速向量表特性）
            newOrigin = StrongerScanOrigin(myPos, tPos, Origin_Radius, Origin_Scans, "Algo1Even")
            newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius, Hit_Scans, "Algo1Even")
        else
            -- 单数轮 -> 偏向 Algo2 的盘面均匀采样（并加入额外前向点）
            local pole = (tPos - myPos)
            local poleUnit = (pole.Magnitude < 0.001) and Vector3.new(0,0,1) or pole.Unit
            newOrigin = StrongerScanOrigin(myPos, tPos, Origin_Radius, Origin_Scans, "Algo2Odd")
            newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius, Hit_Scans, "Algo2Odd")
            -- 原先在单数轮中有插入 forward_offset 的行为，这里 StrongerScanOrigin 已处理
        end
    elseif mode == "RandomSphere" then
        newOrigin = StrongerScanOrigin(myPos, tPos, math.min(SCAN.RandomRadius or 25, 25), Origin_Scans)
        newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius, Hit_Scans)
    elseif mode == "Cardinals" then
        newOrigin = GetOffsets_Cardinals(myPos, Origin_Radius)
        newTarget = GetOffsets_Cardinals(tPos, Hit_Radius)
    elseif mode == "Aggressive" then
        -- Aggressive: 更高密度 + 混合策略（扩大样本量）
        local extraO = math.min(Origin_Scans * 3, 240)
        local extraH = math.min(Hit_Scans * 3, 400)
        newOrigin = StrongerScanOrigin(myPos, tPos, Origin_Radius * 1.2, extraO)
        newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius * 1.2, extraH)
    elseif mode == "Hybrid" then
        newOrigin = StrongerScanOrigin(myPos, tPos, math.max(Origin_Radius*0.7, 0.5), Origin_Scans)
        newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius, Hit_Scans)
    elseif mode == "Focused" then
        -- Focused: 保持小半径高密度
        newOrigin = { myPos, myPos + ((tPos - myPos).Unit * math.min(Origin_Radius*0.6, 10)) }
        for i=1, Origin_Scans*2 do
            table.insert(newOrigin, myPos + GetRandomOffset(Origin_Radius*0.35))
        end
        newTarget = StrongerScanTarget(myPos, tPos, math.max(0.1, Hit_Radius*0.5), Hit_Scans*2)
    else
        -- fallback to alternating original-like behavior but stronger
        if WB.Round % 2 == 0 then
            newOrigin = StrongerScanOrigin(myPos, tPos, Origin_Radius, Origin_Scans, "Algo1Even")
            newTarget = StrongerScanTarget(myPos, tPos, Hit_Radius, Hit_Scans, "Algo1Even")
        else
    ...
    local function ModeHitBoostFactor(mode)
        ...
        if mode == "Aggressive" then return 1.6 end
        if mode == "RandomSphere" then return 1.4 end
        if mode == "Hybrid" then return 1.35 end
        if mode == "Focused" then return 1.6 end
    ...
    if EXPAND_HIT_RADIUS then
        ...
        for i=1,#newTarget do
            ...
            if mag > 0 then
                local scale = math.min(sampleCap/mag, 1)
                newTarget[i] = tPos + dir * scale
            end
        end
    end
    ...
    if bestPO and bestPH then
        ...
        for i = 1, 24 do
            ...
            local pole = (tPos - myPos)
            local arb = math.abs(pole.X) < 0.9 and Vector3.new(1,0,0) or Vector3.new(0,1,0)
            local tt1 = pole:Cross(arb)
            if tt1.Magnitude == 0 then arb = Vector3.new(0,0,1); tt1 = pole:Cross(arb) end
            tt1 = tt1.Unit
            local tt2 = pole:Cross(tt1).Unit
            local offsetVec = tt1 * (math.cos(theta) * r) + tt2 * (math.sin(theta) * r)
            local tryPH = bestPH + offsetVec
            if CheckWallbang(bestPO, tryPH) then
                bestPH = tryPH
                refined = true
                break
            end
        ...
        if not refined then
            for i = 1, 40 do
                local tryPH = bestPH + GetRandomOffset(refineRadius * 1.5)
                local tryPO = bestPO + GetRandomOffset(1.0)
                if CheckWallbang(tryPO, tryPH) then
                    bestPH = tryPH; bestPO = tryPO; refined = true; break
                end
            end
        end
        ...
        if not refined then
            for _, pO in ipairs(newOrigin) do
                if CheckWallbang(pO, tPos) then
                    bestPO = pO; bestPH = tPos; refined = true; break
                end
            end
        end

        if not refined then
            -- 将原先的三次缩放尝试替换为五次以提高稳定性（stable_check 从 3 改为 5）
            for j=1,5 do
                local rr = refineRadius * (1 - j*0.18)
                if rr <= 0 then break end
                for i=1,6 do
                    local theta = math.random() * 2 * math.pi
                    local r = rr * math.sqrt(math.random())
                    local offsetVec = tt1 * (math.cos(theta) * r) + tt2 * (math.sin(theta) * r)
                    local cand = bestPH + offsetVec
                    if CheckWallbang(bestPO, cand) then bestPH = cand; refined = true; break end
                end
                if refined then break end
            end
        end

        if refined then
            Locked_Path={AbsO=bestPO,AbsH=bestPH,Target=target,MyPos=myPos,TPos=tPos,RefineRadius=refineRadius,LastFound=tick(), FastScanUntil = tick() + 0.6, Sampled=true}
            Locked_Path.HitRadiusUsed = refineRadius > 0 and refineRadius or Hit_Radius
...
RunService.Heartbeat:Connect(function()
    ...
    -- 新增：射击距离限制（只在目标在限定距离内才发射），单位：世界单位（米）
    ...
    if Valid_Pair and Valid_Pair.Origin and Valid_Pair.Hit and CheckWallbang(Valid_Pair.Origin, Valid_Pair.Hit) then
        ...
        if Valid_Pair and Valid_Pair.Target and Valid_Pair.Target.Character then
            ...
            if pole.Magnitude > 0.001 then
                ...
                if not found then
                    for j=1,5 do -- stable_check 从 3 改为 5，做更多尝试以提升稳定性
                        local rr = rmax * (1 - j*0.18)
                        if rr <= 0 then break end
                        for i=1,6 do
                            local theta = math.random() * 2 * math.pi
                            local r = rr * math.sqrt(math.random())
    ...
    if HitLogEnabled then
        ...
        if HIT_RECORD.Enabled and fd and fd > 0 then
            local rec = {
                ...
                originY = tostring(Valid_Pair.Origin.Y),
                originZ = tostring(Valid_Pair.Origin.Z),
                hitX = tostring(finalHit.X),
                hitY = tostring(finalHit.Y),
                hitZ = tostring(finalHit.Z),
                cached = tostring(WB.Cached),
                time = tostring(os.time()),
            }
            pcall(function() SaveHitRecord(Valid_Pair.Target.Name, rec) end)
